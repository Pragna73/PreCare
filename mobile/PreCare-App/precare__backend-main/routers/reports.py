...
from services.ai_service import (
    analyze_risk,
    extract_indicators_list,
    extract_mother_age,
    is_pregnancy_medical_report,
)
...
def upload_report(
    ...
    if ext not in {".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".txt", ".md", ".pdf", ".docx", ".doc", ".webp"}:
        raise HTTPException(status_code=400, detail="Unsupported file type. Please upload a PDF, Word document (.docx), or Image.")
    ...
    if not is_pregnancy_medical_report(extracted_text, file.filename or stored_name):
        raise HTTPException(
            status_code=400,
            detail="Invalid Document: The uploaded file does not contain recognized pregnancy or maternal health parameters. Please upload an actual maternal health lab report, blood test, or ultrasound scan.",
        )

    risk = analyze_risk(extracted_text, file.filename or stored_name)
    structured = _parse_structured(extracted_text)

    extracted_patient_name = risk.get("patient_name")
    if extracted_patient_name and extracted_patient_name != "Patient":
        patient_name = extracted_patient_name
    elif user.full_name and user.full_name not in ("Test", "User", "Patient", "sample", "demo", "test"):
    ...
    # Add indicators to structured data map
    for ind in indicators:
        key = ind["name"].lower().replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_")
        structured[key] = ind["value"]
    if "age" in risk:
        structured["age"] = str(risk["age"])
    ...
    return {
        ...
        "auto_actions": outcome["auto_actions"],
        "report_analysis": {
            "extracted_text": extracted_text,
...
def confirm_report_decision(
    ...
    report.confirmation_status = "CONFIRMED" if body.confirm else "DENIED"
    report.requires_confirmation = False
    db.commit()
    ...
    return {
        ...
        "confirmation_status": report.confirmation_status,
        "message": "Action confirmed successfully." if body.confirm else "Action denied successfully.",
    }
...
def get_report_by_id(
    ...
    indicators = extract_indicators_list(report.extracted_text or "")
    age = extract_mother_age(report.extracted_text or "")
