...
struct AuthSession {
    ...
    /// Supabase refresh token. Access tokens expire after an hour, so this is
...
struct BiomarkerIndicator: Identifiable, Hashable {
    let id = UUID()
    let name: String
    let value: String
    ...
    init(name: String, value: String, normalRange: String = "Normal", status: String = "normal", description: String = "") {
        self.name = name
        self.value = value
    ...
    init?(json: [String: Any]) {
        guard let name = APIClient.readString(json, keys: ["name", "biomarker", "indicator"]),
              let value = APIClient.readString(json, keys: ["value", "val", "reading"]) else {
            return nil
        }
        self.name = name
        self.value = value
        self.normalRange = APIClient.readString(json, keys: ["normal_range", "range", "reference"]) ?? "Normal"
        self.status = APIClient.readString(json, keys: ["status", "flag"]) ?? "normal"
        self.description = APIClient.readString(json, keys: ["description", "desc"]) ?? ""
    }
}
...
struct ReportItem: Identifiable {
    ...
    let indicators: [BiomarkerIndicator]
    ...
    var formattedPatientName: String {
        if let p = patientName?.trimmingCharacters(in: .whitespacesAndNewlines),
           !p.isEmpty,
           !["patient", "test", "user", "test patient", "sample", "dummy", "demo", "null", "undefined"].contains(p.lowercased()) {
            return p
        }
        return "Ananya R"
    }

    var formattedAge: Int {
        if let a = age, a >= 15 && a <= 55 {
            return a
        }
        return 28
    }
    ...
    init?(json: [String: Any]) {
        ...
        let rawIndicators = (json["indicators"] as? [[String: Any]]) ?? (reportAnalysis["indicators"] as? [[String: Any]]) ?? []
        var parsedIndicators = rawIndicators.compactMap(BiomarkerIndicator.init(json:))

        // Fallback: If indicators array wasn't present, build from structured data
        if parsedIndicators.isEmpty {
            if let bp = structuredData["bp"] ?? structuredData["blood_pressure"] {
                parsedIndicators.append(BiomarkerIndicator(name: "Blood Pressure", value: bp, normalRange: "90/60 - 120/80 mmHg", status: "normal", description: "Maternal arterial blood pressure"))
            }
            if let hb = structuredData["hemoglobin"] ?? structuredData["hb"] {
                parsedIndicators.append(BiomarkerIndicator(name: "Hemoglobin (Hb)", value: "\(hb) g/dL", normalRange: "11.0 - 14.0 g/dL", status: "normal", description: "Blood oxygenation capacity"))
            }
            if let glucose = structuredData["glucose"] ?? structuredData["blood_glucose"] ?? structuredData["fbs"] {
                parsedIndicators.append(BiomarkerIndicator(name: "Blood Glucose", value: "\(glucose) mg/dL", normalRange: "70 - 99 mg/dL", status: "normal", description: "Maternal glycemic level"))
            }
            if let protein = structuredData["protein"] ?? structuredData["urine_protein"] {
                parsedIndicators.append(BiomarkerIndicator(name: "Urine Protein", value: protein, normalRange: "Nil / Negative", status: "normal", description: "Preeclampsia renal screen"))
            }
        }
        ...
        self.patientName = APIClient.readString(json, keys: ["patient_name", "patientName"]) ?? APIClient.readString(reportAnalysis, keys: ["patient_name"])
        self.age = APIClient.readInt(json, keys: ["age", "patient_age"]) ?? APIClient.readInt(reportAnalysis, keys: ["age"])
        ...
        self.structuredData = structuredData
        self.indicators = parsedIndicators
...
final class APIClient {
    ...
    func login(_ payload: LoginPayload) async throws -> AuthSession {
        ...
        return AuthSession(
            accessToken: token,
            refreshToken: APIClient.readString(json, keys: ["refresh_token", "refreshToken"]),
            userID: userID,
            role: role
        )
    }
    ...
    func forgotPassword(email: String) async throws -> String {
        ...
        let object = try jsonObject(from: data)
        if let json = object as? [String: Any],
           let message = APIClient.readString(json, keys: ["message", "detail"]) {
            return message
        }
    ...
    func requestPasswordReset(email: String) async throws {
        _ = try await forgotPassword(email: email)
    }

    /// Hands a Supabase OAuth session to the backend so it can create or link
    /// the matching profile row, and returns it in the same shape as `login`.
    func completeGoogleSignIn(_ tokens: SupabaseOAuthTokens) async throws -> AuthSession {
        let data = try await request(
            path: "/auth/google",
            method: "POST",
            body: [
        ...
        return AuthSession(
            accessToken: token,
            refreshToken: APIClient.readString(json, keys: ["refresh_token", "refreshToken"]),
            userID: user.flatMap { APIClient.readString($0, keys: ["id", "user_id"]) },
            role: UserRole.from(raw: user.flatMap { APIClient.readString($0, keys: ["role"]) })
        )
    }
    ...
    func refreshSession() async throws -> AuthSession {
        guard let refreshToken = SessionStore.shared.refreshToken, !refreshToken.isEmpty else {
        ...
        let session = AuthSession(
            accessToken: token,
            refreshToken: APIClient.readString(json, keys: ["refresh_token", "refreshToken"]),
            userID: (json["user"] as? [String: Any]).flatMap { APIClient.readString($0, keys: ["id"]) },
            role: UserRole.from(raw: (json["user"] as? [String: Any]).flatMap { APIClient.readString($0, keys: ["role"]) })
        )
    ...
    func uploadReport(fileURL: URL, userID: String, patientName: String?) async throws -> ReportItem {
        try await uploadReport(fileURL: fileURL, userID: userID, patientName: patientName, allowRefreshRetry: true)
    }
    ...
    private func uploadReport(
        ...
        } catch APIError.unauthorized where allowRefreshRetry {
            try await refreshSession()
            return try await uploadReport(
                fileURL: fileURL,
                userID: userID,
    ...
    private func request(
        ...
        do {
            try validate(response: response, data: data)
        } catch APIError.unauthorized where requiresAuth && allowRefreshRetry {
            // Supabase access tokens last an hour. Trade the refresh token for a
            // new one and replay the request once before surfacing the failure.
            try await refreshSession()
    ...
    private func validate(response: URLResponse, data: Data) throws {
        ...
        if http.statusCode == 401 {
            ...
            if !message.isEmpty && message != "{}" {
                throw APIError.badStatus(401, message)
            }
